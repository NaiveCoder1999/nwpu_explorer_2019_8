...
void MapWriterPlugin::initialize(const std::string &name)
{
  ...
  // 这个之前在hector_object_tracker 以及 hector_map_server 中都是有的，那边随便改名
  plugin_nh.param("service_name", service_name_, std::string("/get_object_model"));
  // 这个是自己加的
...
public:
  ...
  void draw(MapWriterInterface *interface)
  {
    ...
    if (description_file.is_open())
    {
      ...
      if (!team_name.empty())
        description_file << "\"" << team_name << "\"" << std::endl;
      if (!country.empty())
        description_file << "\"" << country << "\"" << std::endl;
      description_file << "\"" << now_date << "\"" << std::endl;
      description_file << "\"" << now_time << "\"" << std::endl;
      if (!mission_name.empty())
        description_file << "\"" << mission_name << "\"" << std::endl;
    ...
    int counter = 0;
    ROS_ERROR("in draw start:now the size:%d", data.response.model.objects.size());
         ...
         it != data.response.model.objects.end(); ++it)
    {
      ...
      ROS_ERROR(object.info.class_id.c_str());
 ...
 */
      ...
      // 此处curr的坐标相对于世界坐标系进行标注，坐标轴代表实际长度
      // 比较前后两次点之间是否是重复标记的问题
      // （我感觉消去前一次标记可能是取了平均值，或者）
      // 因为victim只考虑了单镜头的重复标记问题
      // 可能先右摄标记物体后，左侧再一次识别到此物体
      // 所以需要在同一参考系下进行比较
      // 如果不满足条件仍然不进行标注
      double diff = 0;
      diff = (last_pose.x() - coords.x()) * (last_pose.x() - coords.x()) + (last_pose.y() - coords.y()) * (last_pose.y() - coords.y());
      ROS_ERROR("curr_x %f,curr_y %f,distance: %f", coords.x(), coords.y(), diff);
      ...
      //此处的设置了diff的最低距离限制！！！！！！！！！！！！！！！！1
      {
             ...
             tempIt != data.response.model.objects.end(); ++tempIt)
        {
          ...
          ROS_ERROR("%s", compDis.info.class_id.c_str());
...
public:
  ...
  void draw(MapWriterInterface *interface)
  {
    ...
    if (!service_client_.call(data))
    {
      //ROS_ERROR_NAMED(name_, "Cannot draw qrcodes, service %s failed", service_client_.getService().c_str());
      ROS_ERROR("Cannot draw qrcodes, service %s failed", service_client_.getService().c_str());
      return;
    }
 ...
 */
    ...
    for (hector_worldmodel_msgs::ObjectModel::_objects_type::const_iterator it = data.response.model.objects.begin(); it != data.response.model.objects.end(); ++it)
    {
      ...
      if (!draw_all_objects_ && object.state.state != hector_worldmodel_msgs::ObjectState::CONFIRMED)
        continue;
      if (object.state.state == hector_worldmodel_msgs::ObjectState::DISCARDED)
        continue;
...
protected:
  ...
  bool isLargest(const hector_worldmodel_msgs::Object &object, const std::vector<hector_worldmodel_msgs::Object> &objects)
  {
    ...
    for (hector_worldmodel_msgs::ObjectModel::_objects_type::const_iterator it = objects.begin(); it != objects.end(); ++it)
    {
      ...
      float dist_sqr = (object.pose.pose.position.x - object2.pose.pose.position.x) * (object.pose.pose.position.x - object2.pose.pose.position.x) + (object.pose.pose.position.y - object2.pose.pose.position.y) * (object.pose.pose.position.y - object2.pose.pose.position.y);
