    ...
    while (ros::ok())
    {
        if(flag_r)
        {
            ...
            for (auto &i : result_vec) 
            {
                ...
                //    continue;
                ROS_ERROR("--------***-------------------");
                ROS_ERROR("%lf",float((i.x + i.w/2.0)/1280));
                ROS_ERROR("%lf",float((i.y + i.h/2.0)/720));
                ROS_ERROR("%d",i.x);
                ROS_ERROR("%d",i.y);
                ROS_ERROR("%d",i.w);
                ROS_ERROR("%d",i.h);
                ...
                ROS_ERROR("%lf",i.prob);
                 ...
                 //%*(^$*&(*&^))
                //if(float(1.0*i.w/i.h)<1.1 || float(1.0*i.w/i.h) > 1.37 || i.prob < 0.85)
                //    continue;
                //if(float((i.x + i.w/2.0)/1280)<0.41 || float((i.x + i.w/2.0)/1280)>0.61)



               
                if(float((i.x + i.w/2.0)/1280)<0.43 || float((i.x + i.w/2.0)/1280)>0.55)
                    continue;
                if(float((i.y + i.h/2.0)/720)<0.2 || float((i.y + i.h/2.0)/720)>0.8)
                    continue;
                //if(float(1.0*i.w/i.h)<1.32 && float(1.0*i.w/i.h) > 1.24 && i.prob > 0.85)
                //if(1)
                //阈值两次调高
                //{
        ...
        if(flag_l)
        {
            ...
            for (auto &i : result_vec) 
            {
                ...
                if(float((i.x + i.w/2.0)/1280)<0.43 || float((i.x + i.w/2.0)/1280)>0.55)
                    continue;
                if(float((i.y + i.h/2.0)/720)<0.45 || float((i.y + i.h/2.0)/720)>0.65)
                    continue;
                //if(float(1.0*i.w/i.h)<1.32 && float(1.0*i.w/i.h) > 1.22 && i.prob > 0.85)
                //if(1)
                //{
