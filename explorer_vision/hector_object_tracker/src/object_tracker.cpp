...
//对于二维码和受害者采用相同的处理手法，转化为pose消息类型
void ObjectTracker::imagePerceptCb(const hector_worldmodel_msgs::ImagePerceptConstPtr &percept)
{
 ...
 //娃娃检测有收到percept话题，并且在这里正确输出娃娃序号
  ...
  ROS_ERROR("Incoming image percept with image coordinates [%f,%f] in frame %s", percept->x, percept->y, percept->header.frame_id.c_str());
  //ROS_DEBUG 
  //这里测试发送的id是谁
  ...
  CameraModelPtr cameraModel = getCameraModel(percept->header.frame_id, percept->camera_info);
   ...
   ROS_ERROR("->%s   what the fucking is going on????!!!!" ,percept->header.frame_id.c_str() );
  ...
  cv::Point2d rectified = cameraModel->rectifyPoint(cv::Point2d(percept->x, percept->y)); // !!!!
   ...
   what():  Cannot call    when distortion is unknown.
...
void ObjectTracker::posePerceptCb(const hector_worldmodel_msgs::PosePerceptConstPtr &percept)
{
 ...
 ROS_ERROR("In object_tracker.cpp     percept->info.class_id = [%s]",(percept->info.class_id).c_str() );
  ...
  //只有在percept不为qrcode时，pose才会进行发布
  {
...
//变量类型为hector_worldmodel_msgs::ObjectModel（自定义数据类型）
 ...
 ROS_INFO("In objectTracker.cpp    after sendAndResetData");//12
}
