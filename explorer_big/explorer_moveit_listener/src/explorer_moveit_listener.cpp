...
protected:
  ...
  ros::Publisher joint_values_pub; //向下位机发送

  sensor_msgs::JointState joint_state; //节点的状态
  std::string action_name_;

  /**
   * 向下发送的joint_values,包含names和values数组
   * 需要两个值name和value，向下传输的是两个值相减
   **/
  explorer_msgs::explorer_moveit_values joint_values1, joint_values2;
...
public:
  ...
  RobotTrajectoryFollower(std::string name) : nh_("~"),
                                              ...
                                              action_name_(name)
  {
    ...
    //joint_pub = nh_.advertise<sensor_msgs::JointState>("/joint_states", 10);  //向rviz中发布消息来更新模型的状态,已经由robot_hardware进行
    joint_values_pub = nh_.advertise<explorer_msgs::explorer_moveit_values>("explorer_moveit_joint", 50); //向下传递
  ...
  //TODO:需要修改
  //此处用于将explorer_msgs::explorer_moveit_values类型的消息发送下去，这里为最关键的函数
  void joint_states_pub(std::vector<double> joint_posi)
  {
    ...
    joint_values_pub.publish(joint_values1);
  }
