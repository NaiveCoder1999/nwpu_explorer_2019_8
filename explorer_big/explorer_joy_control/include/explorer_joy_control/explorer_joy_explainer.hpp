...
class explorerJoyExplainer {
   public:
    ...
    static int getButtonListSize() { return 0; };
    static int getAxesListSize() { return 0; };
...
class saiTaiKe : public explorerJoyExplainer {
   ...
   public:
    ...
    static int getButtonListSize() { return 15; }
    static int getAxesListSize() { return 8; }
    /**
    ...
    void getMessage(sensor_msgs::Joy::ConstPtr msg) {
        ...
        button.at(left_axes_left) =
            msg->axes[_left_axes_left_right] > threshold;
        button.at(left_axes_right) =
            msg->axes[_left_axes_left_right] < -threshold;
        ...
        button.at(right_axes_left) =
            msg->axes[_right_axes_left_right] > threshold;
        button.at(right_axes_right) =
            msg->axes[_right_axes_left_right] < -threshold;
        button.at(right_axes_up) = msg->axes[_right_axes_up_down] > threshold;
        button.at(right_axes_down) =
            msg->axes[_right_axes_up_down] < -threshold;
        ...
        axes.at(right_button_up_down) =
            msg->buttons[_button1] - msg->buttons[_button3];
        axes.at(right_button_left_right) =
            msg->buttons[_button4] - msg->buttons[_button2];
    }
...
// Another class of SaiTaiKe joystick,此处为目前生效的配置
class shoubing : public explorerJoyExplainer {
   ...
   public:
    ...
    static int getAxesListSize() { return 6; }
...
/** 北通及北通无线手柄的数据与xbox一致，屏蔽
class beitong: public explorerJoyExplainer {
private:
    const double threshold = 0.1;
public:
    beitong() : button(button_size, false), axes(axes_size, 0.0),
explorerJoyExplainer() { _L1 = 4; _L2 = 2; _R1 = 5; _R2 = 5; _button1 = 3;
        _button2 = 1;
        _button3 = 0;
        _button4 = 2;
        _up_down = 7;                  //1.0 上 -1.0 下
    ...
    void getMessage(sensor_msgs::Joy::ConstPtr msg) {
        ...
        button.at(up)       = msg->axes[_up_down]       > threshold;
        button.at(down)     = msg->axes[_up_down]       < -threshold;
        button.at(left_axes_left)   = msg->axes[_left_axes_left_right]  >
threshold; button.at(left_axes_right)  = msg->axes[_left_axes_left_right]  <
-threshold; button.at(left_axes_up)     = msg->axes[_left_axes_up_down]     >
threshold; button.at(left_axes_down)   = msg->axes[_left_axes_up_down]     <
-threshold; button.at(right_axes_left)  = msg->axes[_right_axes_left_right] >
threshold; button.at(right_axes_right) = msg->axes[_right_axes_left_right] <
-threshold; button.at(right_axes_up)    = msg->axes[_right_axes_up_down]    >
threshold; button.at(right_axes_down)  = msg->axes[_right_axes_up_down]    <
-threshold; button.at(left_axes_button) = msg->buttons[_left_axes_button];
        button.at(right_axes_button) = msg->buttons[_right_axes_button];
        ...
        axes.at(right_axes_up_down) = msg->axes[_right_axes_up_down];
        axes.at(right_axes_left_right) = msg->axes[_right_axes_left_right];
        axes.at(right_button_up_down) = msg->buttons[_button1] -
msg->buttons[_button3]; axes.at(right_button_left_right) =
msg->buttons[_button4] - msg->buttons[_button2];
    }
    bool askForButton(int name) {
        // 读取数据
...
class xBox : public explorerJoyExplainer {
   private:
    const double threshold = 0.1;
   ...
   public:
    xBox()
        ...
        _button4 = 2;
        _up_down = 7;               //-32767上
        _left_right = 6;            //-32767左
        _left_axes_up_down = 1;     //-32767上
        _left_axes_left_right = 0;  //-32767左
        _left_axes_button = 9;
        _right_axes_up_down = 4;     //-32767上
        _right_axes_left_right = 3;  //-32767左
        _right_axes_button = 10;
    }
    virtual ~xBox(){};
    static int getButtonListSize() { return 11; }
    static int getAxesListSize() { return 8; }
    ...
    void getMessage(sensor_msgs::Joy::ConstPtr msg) {
        ...
        axes.at(right_button_left_right) = msg->buttons[_button2] - msg->buttons[_button4];
    }
    bool askForButton(int name) {
        // 读取数据
        if (name < 0 || name >= button_size) {
...
class autoJoyExplainer : public explorerJoyExplainer {
   ...
   public:
    autoJoyExplainer() : explorerJoyExplainer() { explainer = new xBox(); }
    virtual ~autoJoyExplainer() {
        if (explainer != NULL) {
            delete explainer;
        }
    };
    int getButtonListSize() { return explainer->getButtonListSize(); }
    int getAxesListSize() { return explainer->getAxesListSize(); }
    ...
    void getMessage(sensor_msgs::Joy::ConstPtr joy) {
        if (joy->buttons.size() != explainer->getButtonListSize() || joy->axes.size() != explainer->getAxesListSize()) {
            if (joy->buttons.size() == saiTaiKe::getButtonListSize() && joy->axes.size() == saiTaiKe::getAxesListSize()) {
                ...
                // ROS_INFO("chose SaiTaiKe Joystick #1");
            } else if (joy->buttons.size() == xBox::getButtonListSize() && joy->axes.size() == xBox::getAxesListSize()) {
                delete explainer;
                explainer = NULL;
                explainer = new xBox();
                // ROS_INFO("chose xBox Joystick");
                // ROS_INFO("chose BeiTong Joystick");
                ...
                // ROS_INFO("chose SaiTaiKe Joystick #2");
            }
            /**else if (joy->buttons.size() ==
            BeiTongWireless::getButtonListSize() && joy->axes.size() ==
            BeiTongWireless::getAxesListSize()) { delete explainer; explainer =
            NULL; explainer = new BeiTongWireless();
    ...
    bool askForButton(int name) { return explainer->askForButton(name); }

    double askForAxes(int name) { return explainer->askForAxes(name); }
};
